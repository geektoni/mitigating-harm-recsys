...
class MyDataset(Dataset):
    def __init__(self, train_file, valid_file, test_file, device,
                 offset=1, alpha=0.2, eigs_dim=64, sample_hop=3):
        self.offset = offset
        self.device = device
        self.alpha = alpha
        ...
        train_pos_data = train_data[train_data[2] < self.offset] # offset indicates positive or negative data
        train_neg_data = train_data[train_data[2] >= self.offset]
        ...
        valid_pos_data = valid_data[valid_data[2] < self.offset]
        valid_neg_data = valid_data[valid_data[2] >= self.offset]
        ...
        test_pos_data = test_data[test_data[2] < self.offset]
        test_neg_data = test_data[test_data[2] >= self.offset]
    ...
    def A_pos(self):
        if self._A_pos is None:
            self._A_pos = torch.sparse_coo_tensor(
                ...
                torch.ones(self.train_pos_user.shape[0]*2).to(self.device),
    ...
    def A_neg(self):
        if self._A_neg is None:
            self._A_neg = torch.sparse_coo_tensor(
                ...
                torch.ones(self.train_neg_user.shape[0]*2).to(self.device),
                torch.Size([self.num_nodes, self.num_nodes]))
        return self._A_neg
    ...
    def tildeA_neg(self):
        if self._tildeA_neg is None:
            ...
            self._tildeA_neg = torch.sparse.mm(
                torch.sparse.mm(D1.to('cpu'), self.A_neg.to('cpu')).to('cpu'), 
                D2.to('cpu')
            ).to(self.device) # Move this operation to CPU and then back to GPU (it is done once anyway)
    ...
    def L(self):
        if self._L is None:
            self._L = (self.L_pos+self.alpha*self.L_neg)/(1+self.alpha)
        return self._L
    ...
    def L_eigs(self):
        if self._L_eigs is None:
            if self.eigs_dim == 0:
                ...
                self._L_eigs = F.layer_norm(self._L_eigs, normalized_shape=(self.eigs_dim,))
        return self._L_eigs
    ...
    def sample(self):
        if self._indices is None:
            ...
            self._paths = torch.cat([
                ...
                torch.zeros(self.train_neg_user.shape).repeat(2)], dim=0).long().to(self.device)
            ...
            self._values = torch.ones(self._indices.shape[1]).to(
                self.device)*d[self._indices[0]]*d[self._indices[1]]
        res_X, res_Y = [], []
        record_X = []
        ...
        for hop in range(self.sample_hop):
            ...
            next_indices = self._counts_sum[X[1]]-(torch.rand(X.shape[1]).to(self.device)*self._counts[X[1]]).long()-1
